...
[![Tests](https://img.shields.io/badge/tests-22%20passing-brightgreen.svg)](#测试与构建)
...
- **中文 Prompt 工程**：所有角色固定前置公共规则与角色简化规则，强制严格 JSON 输出、发言不超过 100 个汉字。每种动作都会给出**含真实字段的 JSON 示例**（如 `targetSeat`/`poisonSeat`/`useAntidote`），并明确要求座位号必须写入结构化字段、不得只写进 `reasoning`，避免击杀/查验目标丢失而退化为意外空刀/弃验。
...
```bash
mvn test          # 22 个测试：状态机流转、夜晚结算、平票裁决、胜负判定、
