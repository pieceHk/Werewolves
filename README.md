...
- 夜晚结算：狼人可空刀、预言家可弃验、女巫解药/毒药；女巫固定不能自救、不能同晚双药。
- 第 1 天警长竞选：上警、退水、警下投票、平票重投、警徽流失。
- 白天投票：多数票、警长 1.5 票、平票重投、二次平票无人出局。
- 胜负判定：好人胜、狼人胜，支持屠边/屠城规则配置。
- Agent 信息隔离：公开事件、私有事件、座位 audience、每局私有记忆。
- LLM 接入：离线 `MockLLMClient` 与 DashScope/Qwen OpenAI 兼容接口。
- 对局输出：上帝视角日志、每个 Agent 私有日志、回放 JSON、元数据 JSON、每轮表现分、全局复盘。
